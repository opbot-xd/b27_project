...
type userReq struct {
	...
	Client   string `json:"client"` //client type
}

type response struct {
	Status  bool        `json:"status"`
	Message string      `json:"message"`
	Data    interface{} `json:"data,omitempty"` //can store any datatype
	Total   int         `json:"total,omitempty"` //total number of records in the response
}

func registerHandler(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json") //w is response and set to json type

	u := &userReq{}   // user request is decoded below and store
...
func loginHandler(w http.ResponseWriter, r *http.Request) {
	...
	//the decoded userReq struct is sent to login function of redisrepo.go
	res := login(u)
	json.NewEncoder(w).Encode(res)
}
...
func verifyContactHandler(w http.ResponseWriter, r *http.Request) {
	...
	//takes the username out of the user's request and sends to verifyContact function
...
func chatHistoryHandler(w http.ResponseWriter, r *http.Request) {
	...
	//retrives fromTs and toTs from url and sends u1 u2 tots fromts
...
func contactListHandler(w http.ResponseWriter, r *http.Request) {
	...
	//gets contact list of a particular user
...
func chatHistory(username1, username2, fromTS, toTS string) *response {
	...
	//uses response struct to send data
	...
	//calls redisrepo function to get the chats between user1 and user2
